...
//ПОКЛИНГТОН
vector<bint> poklingtonOtbros;

bool poklingtonTest(bint n, int t, const vector<int>& qs) {
    uniform_int_distribution<bint> dis(2, n-1);
    vector<bint> randomBint;
    for (int i = 0; i < t; i++) {
        bint elem;
...
pair<bint, vector<int>> poklingtonGen(int k) {
    bint F = 1;
    uniform_int_distribution<> dis(0, simpleNums.size() - 1);
    uniform_int_distribution<> dis2(1, 3);
...
bint poklington(int k, int t) {
    bint final;
    while (true) {
        auto [n, qs] = poklingtonGen(k);
        final = n;
        if (poklingtonTest(n, t, qs)) {break;} else {poklingtonOtbros.push_back(n);}
    }
    return final;
}

void TestAlgPoklington() {
    map<int, vector<int>> tests = {
        {13, {2}},
        {29, {7}},
        {61, {3, 5}},
...
void tables() {
    ...
    //================================ТЕСТЫ НА ПОKЛИНГТОНА
    vector<bint> polingtonGens;
    vector<vector<bint>> polingtonSost;
    for (int i = 0; i < 10; i++) {
        poklingtonOtbros.clear();
        polingtonGens.push_back(poklington(3, 5));
        polingtonSost.push_back(poklingtonOtbros);
    }
